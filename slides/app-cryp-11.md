...
date: Lecture Week 11 -- Mon 08 December 2025
transition: fade
theme: killian
width: 1920
height: 1080
...
* Recall Fermat's Little Theorem: If $p$ is a prime, and $a$ an integer coprime to $p$, then 
...
* Select a random $a$, satisfying $1 \lt a \lt n-1$
* Proceed through the list 
$$a^q, a^{2q}, a^{2^2 q} , a^{2^3 q}, \dots , a^{2^{(k-1)} q} \quad \pmod{n},$$
...
## Correction to Stallings

* The Miller-Rabin test is discussed in detail in <a href="https://mmu.on.worldcat.org/oclc/1334132058" target="_blank">Section 2.6 in Stallings</a>.
* On page 67 there is a pseudo-code definition for a function `Test(n)` implementing the test.
* Unfortunately the typesetting is not correct there. I think this is a result of the conversion to a pdf ebook perhaps?
* Here is a corrected version of that pseudo-code. Not that the if a `return` command executes, the function `Test` will cease executing and pass control back to whatever code or user invoked the function.  
